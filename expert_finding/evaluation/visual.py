...
def plot_evaluations(evals, prefix="no_prefix", path_visuals=None):
    ...
    for t, eval in evals.items():
        precision, recall, thresholds_pr = eval["all"]["curves"]["precision"], eval["all"]["curves"]["recall"], \
                                           eval["all"]["curves"]["thresholds_pr"]
        fpr, tpr, thresholds_roc = eval["all"]["curves"]["fpr"], eval["all"]["curves"]["tpr"], eval["all"]["curves"][
            "thresholds_roc"]
        ###############
        # PRECISION CURVE
        ###############
        ...
        mean_pre[-1] = 0.0
        axarr[0].plot(mean_rec, mean_pre, color=matplotlib.colors.to_hex(colors[i]),
                      label=r'Mean PR for model "{0}" (AP={1:.3f})'.format(t, float(eval["all"]["metrics"]["AP"])))
        ...
        mean_tpr[-1] = 1.0
        axarr[1].plot(mean_fpr, mean_tpr, color=matplotlib.colors.to_hex(colors[i]),
                      label=r'Mean ROC for topic "{0}" (Mean AUC={1:.3f})'.format(t, float(
                          eval["all"]["metrics"]["ROC AUC"])))
...
def plot_evaluation(eval, prefix="no_prefix", path_visuals=None, parameters=None):
    record_evaluation(eval, prefix, parameters=parameters)
    ...
    # print("Path  : ", path) // vis path
    # plt.savefig(path)


def record_evaluation(eval, prefix, hybird=None, parameters=None):
    title = parameters['data_type'] + ":/" + parameters['query_type'] + "->"
    title += " - ".join([i + "={0:.3f}".format(j) for i, j in eval["metrics"].items()])
    res = parameters['model_name'] + "->" + ":" + title + "\n"
    file = parameters['input_dir'] + "/" + "res"
...
def plot_ROC_topics(evals, prefix="no_prefix", path_visuals=None):
    ...
    for t, eval in evals.items():
        ...
        plt.plot(mean_fpr, mean_tpr,
                 label=r'Mean ROC for topic "{0}" (Mean AUC={1:.3f})'.format(t, float(eval["metrics"]["ROC AUC"])))
        std_tpr = np.std(tprs, axis=0)
        tprs_upper = np.minimum(mean_tpr + std_tpr, 1)
        tprs_lower = np.maximum(mean_tpr - std_tpr, 0)
...
def plot_PreRec_topics(evals, prefix="no_prefix", path_visuals=None):
    ...
    for t, eval in evals.items():
        ...
        plt.plot(mean_rec, mean_pre,
                 label=r'Mean PR for topic "{0}" (AP={1:.3f})'.format(t, float(eval["metrics"]["AP"])))
